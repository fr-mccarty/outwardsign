...
async function setupUITestUser() {
  console.log('Setting up persistent test user for Playwright UI...\n');
  ...
  try {
    ...
    console.log('1. Checking if UI test user exists...');
    const { data: existingUsers } = await supabase.auth.admin.listUsers();
    const existingUser = existingUsers?.users?.find(u => u.email === UI_TEST_EMAIL);

    if (existingUser) {
      userId = existingUser.id;
      console.log(`   [OK] User already exists with ID: ${userId}`);
    ...
    if (!parishId) {
      ...
      console.log('   [OK] User linked to parish with staff role');
      ...
      console.log('   [OK] User settings configured');
    }

    // 5. Save credentials to .env.test
    console.log('\n5. Saving credentials to .env.test...');
...
`;
    ...
    console.log('   [OK] Credentials saved to .env.test');

    console.log('\nUI test user setup complete!\n');
    console.log('=======================================');
    console.log(`Email: ${UI_TEST_EMAIL}`);
    console.log(`Password: ${UI_TEST_PASSWORD}`);
    console.log(`Parish: ${UI_TEST_PARISH_NAME}`);
    console.log(`User ID: ${userId}`);
    console.log(`Parish ID: ${parishId}`);
    console.log(`Role: staff`);
    console.log('=======================================\n');
    console.log('[OK] You can now run Playwright UI:');
