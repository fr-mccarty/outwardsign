...
interface MassExportButtonGroupProps {
  massId: string
  scriptId: string
}

/**
 * MassExportButtonGroup Component
 *
 * Displays 4 stacked export buttons for Mass scripts:
 * - PDF: Download PDF file
 * - Word: Download .docx file
 * - Print: Open print view in new tab
 * - Text: Download .txt file
 *
 * Mirrors the ExportButtonGroup component but uses Mass-specific API routes.
 */
export function MassExportButtonGroup({ massId, scriptId }: MassExportButtonGroupProps) {
  const handlePDFExport = async () => {
    try {
      const response = await fetch(`/api/masses/${massId}/scripts/${scriptId}/export/pdf`)
      if (!response.ok) {
  ...
  const handleWordExport = async () => {
    try {
      const response = await fetch(`/api/masses/${massId}/scripts/${scriptId}/export/docx`)
      if (!response.ok) {
  ...
  const handlePrint = () => {
    window.open(`/print/masses/${massId}/scripts/${scriptId}`, '_blank')
  }

  const handleTextExport = async () => {
    try {
      const response = await fetch(`/api/masses/${massId}/scripts/${scriptId}/export/txt`)
      if (!response.ok) {
