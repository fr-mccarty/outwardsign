/**
 * Group Baptism Summary (English) Template
 *
 * Simple summary of group baptism information showing all children
 */
...
/**
 * Main builder function for group baptism summary template (English)
 */
export function buildSummaryEnglish(groupBaptism: GroupBaptismWithRelations): LiturgyDocument {
  const elements: ContentElement[] = []

  // Section title
  elements.push({
    type: 'section-title',
    text: 'Baptisms in This Group'
  })
  ...
  if (groupBaptism.baptisms && groupBaptism.baptisms.length > 0) {
    groupBaptism.baptisms.forEach((baptism, index) => {
      ...
      if (baptism.mother || baptism.father) {
        const parents = [baptism.mother?.full_name, baptism.father?.full_name]
          .filter(Boolean)
          .join(' and ')
        valueLines.push(`Parents: ${parents}`)
      }
      ...
      if (baptism.sponsor_1 || baptism.sponsor_2) {
        const sponsors = [baptism.sponsor_1?.full_name, baptism.sponsor_2?.full_name]
          .filter(Boolean)
          .join(' and ')
        valueLines.push(`Godparents: ${sponsors}`)
      }
    ...
    elements.push({
      type: 'text',
      text: 'No baptisms have been added to this group yet.'
    })
  }
  ...
  if (groupBaptism.group_baptism_event?.start_date) {
    ...
    if (groupBaptism.group_baptism_event.start_time) {
      subtitle += ` at ${formatTime(groupBaptism.group_baptism_event.start_time)}`
    }
  }
  ...
  return {
    ...
    type: 'baptism',
    language: 'en',
    template: 'group-baptism-summary-english',
    title: groupBaptism.name || 'Group Baptism',
    subtitle,
    sections: [
      {
        id: 'baptisms-list',
        title: 'Group Baptism Summary',
        elements
      }
    ]
  }
