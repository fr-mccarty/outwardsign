...
const listGroups: CategorizedTool = {
  name: 'list_groups',
...
const getGroup: CategorizedTool = {
  name: 'get_group',
...
const getPersonGroups: CategorizedTool = {
...
const addToGroup: CategorizedTool = {
  name: 'add_to_group',
...
const removeFromGroup: CategorizedTool = {
  ...
  async execute(args, context) {
    ...
    const groupName = (membership.group as unknown as { name: string } | null)?.name || 'group'
...
const updateGroupMemberRole: CategorizedTool = {
  ...
  async execute(args, context) {
    ...
    const personName = (membership.person as unknown as { full_name: string } | null)?.full_name || 'Member'
...
const leaveGroup: CategorizedTool = {
  ...
  async execute(args, context) {
    ...
    const groupName = (membership.group as unknown as { name: string } | null)?.name || 'the group'
...
export const groupsTools: CategorizedTool[] = [
  listGroups,
  getGroup,
  getPersonGroups,
  getMyGroups,
