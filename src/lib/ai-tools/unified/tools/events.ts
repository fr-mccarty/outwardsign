...
const listEvents: CategorizedTool = {
  name: 'list_events',
...
const getCalendarEvents: CategorizedTool = {
...
const getPublicCalendar: CategorizedTool = {
  ...
  async execute(args, context) {
    ...
    const publicEvents = (data || []).filter((event) => {
      const masterEvent = event.master_event as unknown as {
        event_type: { show_on_public_calendar: boolean } | null
      } | null
...
const getMySchedule: CategorizedTool = {
  ...
  async execute(args, context) {
    ...
    const upcomingAssignments = (assignments || []).filter((assignment) => {
      const calEvent = assignment.calendar_event as unknown as { start_datetime: string } | null
    ...
    upcomingAssignments.sort((a, b) => {
      const dateA = (a.calendar_event as unknown as { start_datetime: string } | null)?.start_datetime || ''
      const dateB = (b.calendar_event as unknown as { start_datetime: string } | null)?.start_datetime || ''
...
export const eventsTools: CategorizedTool[] = [
  listEvents,
  getCalendarEvents,
  getPublicCalendar,
  getMySchedule,
