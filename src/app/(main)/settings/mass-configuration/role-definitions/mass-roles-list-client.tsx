...
export function MassRolesListClient({ massRoles: initialData, userParish }: MassRolesListClientProps) {
  ...
  const handleSearchChange = (value: string) => {
    ...
    router.push(`/settings/mass-configuration/role-definitions${value ? `?${params.toString()}` : ""}`)
  }
  ...
  return (
    <PageContainer
      ...
      actions={
        canManageRoles ? (
          <Link href="/settings/mass-configuration/role-definitions/create">
            <Button>
        ...
        <div className="flex flex-col gap-2 overflow-hidden">
          {filteredItems.map((role) => (
            <DraggableListItem
              ...
              editHref={`/settings/mass-configuration/role-definitions/${role.id}/edit`}
              viewHref={`/settings/mass-configuration/role-definitions/${role.id}`}
