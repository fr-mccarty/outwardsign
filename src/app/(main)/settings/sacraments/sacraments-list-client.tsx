...
interface SacramentsListClientProps {
...
export function SacramentsListClient({ initialData }: SacramentsListClientProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success('Sacrament type deleted successfully')
      setEventTypeToDelete(null)
      router.refresh()
    } catch (error) {
      console.error('Failed to delete sacrament type:', error)
      const errorMessage = error instanceof Error ? error.message : 'Failed to delete sacrament type'
  ...
  return (
    <PageContainer
      ...
      primaryAction={
        <Button asChild>
          <Link href="/settings/sacraments/create">
            ...
            <div className="space-y-2">
              ...
              {items.length === 0 && (
                <div className="text-center py-12">
                  <p className="font-medium mb-2">{t('noSacraments')}</p>
                  <p className="text-muted-foreground mb-4 max-w-md mx-auto">
                    {t('noSacramentsMessage')}
                  </p>
                  <Button asChild>
                    <Link href="/settings/sacraments/create">
                      <Plus className="h-4 w-4 mr-2" />
