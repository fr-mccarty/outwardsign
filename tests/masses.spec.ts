...
test.describe('Masses Module', () => {
  test('should create, view, edit, and verify print view for a mass', async ({ page }) => {
    ...
    // URL is /mass-liturgies/[id]/edit, so we need the second-to-last segment
    ...
    await page.goto(`/print/mass-liturgies/${massId}`);
    await expect(page).toHaveURL(`/print/mass-liturgies/${massId}`, { timeout: TEST_TIMEOUTS.NAVIGATION });
  ...
  test('should update mass and verify persistence after page refresh', async ({ page }) => {
    ...
    // Get mass ID from URL (URL is /mass-liturgies/[id]/edit)
    const urlParts = page.url().split('/');
    const massId = urlParts[urlParts.length - 2];
