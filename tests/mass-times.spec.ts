...
test.describe('Mass Times Module', () => {
  test('should create and view a mass times template', async ({ page }) => {
    ...
    await page.waitForURL(/\/mass-times\/[a-f0-9-]+$/, { timeout: TEST_TIMEOUTS.EXTENDED });
  ...
  test('should edit an existing template', async ({ page }) => {
    ...
    await page.goto(`/mass-times/${templateId}/edit`);
    await expect(page).toHaveURL(`/mass-times/${templateId}/edit`);
